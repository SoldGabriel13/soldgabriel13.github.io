...
if (params.get("kb") === "true") {
console.log("keyboard input is set to true!");

const actions = [
  { keys: new Set(["a"]), run: async (mod) => speak(`${mod}A`) },
  { keys: new Set(["t", "h"]), run: async (mod) => speak(`${mod}TH`) },
  { keys: new Set(["a", "y"]), run: async (mod) => speak(`${mod}AAIH`) },
  { keys: new Set(["a", "i", "h"]), run: async (mod) => speak(`${mod}AAIH`) },
  { keys: new Set(["k"]), run: async (mod) => speak(`${mod}K`) },
  { keys: new Set(["g"]), run: async (mod) => speak(`${mod}G`) },
  { keys: new Set(["g", "a"]), run: async (mod) => speak(`${mod}G A`) },
  { keys: new Set(["s"]), run: async (mod) => speak(`${mod}S`) },
  { keys: new Set(["p"]), run: async (mod) => speak(`${mod}P`) },
  { keys: new Set(["b"]), run: async (mod) => speak(`${mod}B`) },
  { keys: new Set(["t"]), run: async (mod) => speak(`${mod}T`) },
  { keys: new Set(["d", "x"]), run: async (mod) => speak(`${mod}r-50 D r`) },
  { keys: new Set(["d"]), run: async (mod) => speak(`${mod}D`) },
  { keys: new Set(["u", "h"]), run: async (mod) => speak(`${mod}UH`) },
  { keys: new Set(["l", "r"]), run: async (mod) => speak(`${mod}( DX DX DX DX )`) },
  { keys: new Set(["1", "j"]), run: async (mod) => speak(`${mod}r-50 J r`) },
  { keys: new Set(["i", "h"]), run: async (mod) => speak(`${mod}IH`) },
  { keys: new Set(["i"]), run: async (mod) => speak(`${mod}I`) },
  { keys: new Set(["i", "n"]), run: async (mod) => speak(`${mod}IN`) },
  { keys: new Set(["e", "n"]), run: async (mod) => speak(`${mod}EN`) },
  { keys: new Set(["a", "e"]), run: async (mod) => speak(`${mod}AE`) },
  { keys: new Set(["a", "n"]), run: async (mod) => speak(`${mod}AN`) },
  { keys: new Set(["o", "n"]), run: async (mod) => speak(`${mod}ON`) },
  { keys: new Set(["e"]), run: async (mod) => speak(`${mod}E`) },
  { keys: new Set(["e", "y"]), run: async (mod) => speak(`${mod}EIH`) },
  { keys: new Set(["e", "h"]), run: async (mod) => speak(`${mod}EH`) },
  { keys: new Set(["e", "i", "h"]), run: async (mod) => speak(`${mod}EIH`) },
  { keys: new Set(["e", "r"]), run: async (mod) => speak(`${mod}ER`) },
  { keys: new Set(["r"]), run: async (mod) => speak(`${mod}R`) },
  { keys: new Set(["m"]), run: async (mod) => speak(`${mod}M`) },
  { keys: new Set(["n"]), run: async (mod) => speak(`${mod}N`) },
  { keys: new Set(["h"]), run: async (mod) => speak(`${mod}H`) },
  { keys: new Set(["o"]), run: async (mod) => speak(`${mod}O`) },
  { keys: new Set(["o", "h"]), run: async (mod) => speak(`${mod}OH`) },
  { keys: new Set(["l"]), run: async (mod) => speak(`${mod}L`) },
  { keys: new Set(["u"]), run: async (mod) => speak(`${mod}U`) },
  { keys: new Set(["u", "n"]), run: async (mod) => speak(`${mod}UN`) },
  { keys: new Set(["v"]), run: async (mod) => speak(`${mod}V`) },
  { keys: new Set(["w"]), run: async (mod) => speak(`${mod}W`) },
  { keys: new Set(["j", "h"]), run: async (mod) => speak(`r-50 ${mod}JH`) },
  { keys: new Set(["j"]), run: async (mod) => speak(`${mod}J`) },
  { keys: new Set(["z"]), run: async (mod) => speak(`${mod}Z`) },
  { keys: new Set(["f"]), run: async (mod) => speak(`${mod}F`) },
  { keys: new Set(["z", "h"]), run: async (mod) => speak(`${mod}ZH`) },
  { keys: new Set(["s", "h"]), run: async (mod) => speak(`${mod}SH`) },
  { keys: new Set(["c", "h"]), run: async (mod) => speak(`${mod}CH`) },
];

const pressed = new Set();
const cooldown = new Map();

function canFire(name, delay = 10) {
  const now = Date.now();
  if (cooldown.get(name) && now - cooldown.get(name) < delay) return false;
...
function getMatches() {
  return actions.filter(action =>
    [...action.keys].every(k => pressed.has(k))
  );
}

function getModifierPrefix(e) {
  if (e.shiftKey) return "b+5 ";
  if (e.altKey) return "b-5 ";
  return "";
}
...
window.addEventListener("keydown", (e) => {
  ...
  const maxSize = Math.max(...matches.map(a => a.keys.size));
  const best = matches.filter(a => a.keys.size === maxSize);

  const mod = getModifierPrefix(e);

  for (const action of best) {
    const id = mod + [...action.keys].sort().join("+");
